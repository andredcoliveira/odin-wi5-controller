    ...
    @Override public void run() {
        ...
        while (true) {
            try {
                ...
                System.out.println(
                        "[ShowMatrixOfDistancedBs] Matrix of Distance");
                System.out.println(
                ...
                System.out.println(
                        "[ShowMatrixOfDistancedBs] Scanning channel "
                        + SCANN_PARAMS.channel);
                System.out.println("[ShowMatrixOfDistancedBs]");

                for (InetAddress beaconAgentAddr : getAgents()) {
                    scanningAgents.clear();
                    System.out.println(
                            "[ShowMatrixOfDistancedBs] Agent to send measurement beacon: "
                            + beaconAgentAddr);

                    // For each Agent
                    System.out.println(
                            "[ShowMatrixOfDistancedBs] Request for scanning during the interval of  "
                            + SCANN_PARAMS.scanning_interval + " ms in SSID "
                            + SCANNED_SSID);
                    for (InetAddress agentAddr : getAgents()) {
                        if (agentAddr != beaconAgentAddr) {
                            System.out.println(
                                    "[ShowMatrixOfDistancedBs] Agent: "
                                    + agentAddr);

                            // Request distances
                            result = requestScannedStationsStatsFromAgent(
                                    agentAddr, SCANN_PARAMS.channel,
                                    SCANNED_SSID);
                    ...
                    if (requestSendMesurementBeaconFromAgent(beaconAgentAddr,
                        ...
                        == 0) {
                        System.out.println(
                                "[ShowMatrixOfDistancedBs] Agent BUSY during measurement beacon operation");
                        continue;
                    }
                    ...
                    for (InetAddress agentAddr : getAgents()) {
                        if (agentAddr != beaconAgentAddr) {
                            ...
                            System.out.println(
                                    "[ShowMatrixOfDistancedBs] Agent: "
                                    + agentAddr + " in channel "
                                    + SCANN_PARAMS.channel);

                            // Reception distances
                            if (scanningAgents.get(agentAddr) == 0) {
                                System.out.println(
                                        "[ShowMatrixOfDistancedBs] Agent BUSY during scanning operation");
                                continue;
                            }
