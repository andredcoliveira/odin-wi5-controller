    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                System.out.println(
                        "[ShowMatrixOfDistancedBs] Scanning channel " + SCANN_PARAMS.channel);
                System.out.println("[ShowMatrixOfDistancedBs]");
                ...
                for (InetAddress beaconAgentAddr : getAgents()) {
                    ...
                    for (InetAddress agentAddr : getAgents()) {
                        if (agentAddr != beaconAgentAddr) {
                            ...
                            result = requestScannedStationsStatsFromAgent(agentAddr,
                                    SCANN_PARAMS.channel, SCANNED_SSID);
                            scanningAgents.put(agentAddr, result);
                        }
                    }

                    // Request to send measurement beacon
                    if (requestSendMesurementBeaconFromAgent(beaconAgentAddr, SCANN_PARAMS.channel,
                            SCANNED_SSID) == 0) {
                        System.out.println(
                    ...
                    for (InetAddress agentAddr : getAgents()) {
                        if (agentAddr != beaconAgentAddr) {
                            ...
                            System.out.println("[ShowMatrixOfDistancedBs]");
                            System.out.println(
                                    "[ShowMatrixOfDistancedBs] Agent: " + agentAddr + " in channel "
