...
@SuppressWarnings("Duplicates") public class SmartApSelection
    ...
    /**
     * Register flow detection Configure the detection of flows
     */
     ...
     * another AP with better received Rssi and a previous hand off has
     * not happened in the last 4000 ms
     */
    @Override public void run() {

        System.out.println("[SmartAPSelection] Start");
        // Import the parameters of Poolfile, using this function of Odin Master
        this.SMARTAP_PARAMS = getSmartApSelectionParams();

        // Wait a period in order to let the user start the agents
        try {
            System.out.println("[SmartAPSelection] Sleep for "
        ...
        int[] channelsAux = new int[num_agents]; // Channels in use by the auxiliary interfaces of each AP

        // Add this information to the log file
        ps.println("[SmartAPSelection] Log file "
                   + SMARTAP_PARAMS.filename); // Log in file
        ps.println("[SmartAPSelection] Parameters:");
        ps.println("\tTime_to_start: " + SMARTAP_PARAMS.time_to_start);
        ps.println(
                "\tScanning_interval: " + SMARTAP_PARAMS.scanning_interval);
        ps.println("\tAdded_time: " + SMARTAP_PARAMS.added_time);
        ps.println("\tSignal_threshold: " + SMARTAP_PARAMS.signal_threshold);
        ps.println("\tHysteresis_threshold: "
                   + SMARTAP_PARAMS.hysteresis_threshold);
        ps.println(
                "\tPrevious_data_weight (alpha): " + SMARTAP_PARAMS.weight);
        ...
        StringBuilder showAPsLine = new StringBuilder(
                "\033[K\r[SmartAPSelection] ");
        ...
        for (InetAddress agentAddr : agents) {
            ...
            String hostIP = agentAddr
                    .getHostAddress(); // Build line for user interface
            showAPsLine.append("\033[0;1m[ AP")
                       .append(hostIP.substring(hostIP.lastIndexOf('.') + 1))
                       .append(" ]");

            int chann = getChannelFromAgent(agentAddr);
            // System.out.println("[SmartAPSelection] = Channels " + Arrays.toString(channels));
            Arrays.sort(
                    channelsAux); // ordered list of channels used by the main interface of the APs. A channel is only added once
            // System.out.println("[SmartAPSelection] = Search for " + chann + ": " +
            // Arrays.binarySearch(channelsAux, chann));

            if (Arrays.binarySearch(channelsAux, chann)
                < 0) { // if already in array, not necessary to add it
                channelsAux[num_channels] = chann;
                channels[num_channels] = chann;
                // System.out.println("[SmartAPSelection] Chann added " + chann);
            }
            System.out.println(
                    "[SmartAPSelection] AP " + agentAddr + " in channel: "
                    + chann);
            ps.println("[SmartAPSelection] AP " + agentAddr + " in channel: "
                       + chann); // Log in file
            ...
            try {
                ...
                if (num_clients == 0) { // No clients, no need of scan
                    System.out.println(
                            "\033[K\r[SmartAPSelection] ====================");
                    System.out.println(
                            "\033[K\r[SmartAPSelection] Proactive AP Handoff");
                    System.out.println("\033[K\r[SmartAPSelection]");
                    System.out.println(
                            "\033[K\r[SmartAPSelection] " + progressChar[
                                    progressIndex++ % 4]
                ...
                for (OdinClient oc : clients) { // Create array with client channels and their indexes for better data processing
                    ...
                    client_channel = getChannelFromAgent(
                ...
                for (int channel = 0; channel < num_channels; ++channel) {
                    ...
                    for (InetAddress agentAddr : agents) {
                        ...
                        result = requestScannedStationsStatsFromAgent(
                                agentAddr, channels[channel], SCANNED_SSID);
                ...
                InetAddress[] agentsArray = agents
                ...
                for (OdinClient oc : clients) {
                    ...
                    for (ind_aux = 0;
                         ind_aux < num_agents; ind_aux++) { // For

                        String arr = vals_rx[client_channel][ind_aux]; // String with "MAC rssi\nMAC rssi\n..."

                        Double rssi = getRssiFromRxStats(eth,
                                                         arr); // rssi or -99.9

                        // get the stored RSSI averaged value (historical data)
                        Double[] client_average_dBm = rssiData.get(eth);

                        if (client_average_dBm
                            == null) { // First time STA is associated

                            client_average_dBm = new Double[num_agents];
                            Arrays.fill(client_average_dBm,
                                        -99.9); // first, we put -99.9 everywhere
                            client_average_dBm[ind_aux] = rssi; // put the last value instead of -99.9

                        } else {

                            // Recalculate the new average
                            if ((client_average_dBm[ind_aux] != null) && (
                                    client_average_dBm[ind_aux] != -99.9)) {
                                if (rssi != -99.9) {
                                    double client_signal = Math.pow(10.0,
                                                                    (rssi)
                                                                    / 10.0); // Linear power
                                    double client_average = Math.pow(10.0,
                                                                     (client_average_dBm[ind_aux])
                                                                     / 10.0); // Linear power average
                                    client_average = client_average * (1
                                                                       - SMARTAP_PARAMS.weight)
                                                     + client_signal
                                                       * SMARTAP_PARAMS.weight; // use the "alpha" parameter (weight) to calculate the new value
                                    client_average_dBm[ind_aux] =
                                            (double) Math.round(1000 * Math
                                                    .log10(client_average))
                                ...
                                setStaWeightedRssiForAgent(eth,
                                                           agentsArray[ind_aux],
                                                           rssi);  // ADDED; TODO: Test
                            }
                        }
                        rssiData.put(eth,
                                     client_average_dBm); // Store all the data
                    }
                    client_index++;
                }
                System.out.println(
                        "\033[K\r[SmartAPSelection] Processing done in: " + (
                                System.currentTimeMillis() - time) + " ms");
                System.out.println(
                        "\033[K\r[SmartAPSelection] ====================");
                System.out.println("\033[K\r[SmartAPSelection] ");
                ...
                for (OdinClient oc : clients) {
                    ...
                    if (clientAddr
                            .equals(nullAddr)) { // If client not assigned, go to next one (associated, but without IP address)
                        continue;
                    }

                    System.out.println(
                            "\033[K\r[SmartAPSelection] \t\t\t\tClient "
                            + clientAddr + " in agent " + agentAddr);
                    ps.println("\tClient " + clientAddr + " in agent "
                               + agentAddr); // Log in file
                    ...
                    if (client_dBm != null) { // Array with rssi
                        ...
                        for (ind_aux = 1; ind_aux
                                          < client_dBm.length; ind_aux++) { // Get the index of the AP where the STA has the highest RSSI, VIP AP
                            // not considered

                            if ((client_dBm[ind_aux] > maxRssi)
                                && (!vipAPAddr
                                    .equals(agentsArray[ind_aux]))) {
                        ...
                        for (ind_aux = 0;
                             ind_aux < client_dBm.length; ind_aux++) {

                            if (agentsArray[ind_aux]
                                    .equals(agentAddr)) { // Current AP

                                currentRssi = client_dBm[ind_aux];
                                if (agentsArray[ind_aux].equals(vipAPAddr)) {
                                    System.out
                                            .print("[\033[48;5;3;1m" + String
                                                    .format("%.2f",
                                                            client_dBm[ind_aux])
                                                   + "\033[00m]"); // Olive
                                } else {
                                    System.out.print("[\033[48;5;29;1m"
                                                     + String.format("%.2f",
                                                                     client_dBm[ind_aux])
                                                     + "\033[00m]"); // Dark Green
                                }
                                ps.println("\t\t[Associated] Rssi in agent "
                                           + agentsArray[ind_aux] + ": "
                                           + client_dBm[ind_aux]
                                           + " dBm"); // Log in file
                                ...
                                if (ind_aux
                                    == client_index) { // Max, VIP AP not considered

                                    System.out
                                            .print("[\033[48;5;88m" + String
                                                    .format("%.2f",
                                                            client_dBm[ind_aux])
                                                   + "\033[00m]"); // Dark red
                                    ps.println(
                                            "\t\t[BetterAP] Rssi in agent "
                                            + agentsArray[ind_aux] + ": "
                                            + client_dBm[ind_aux]
                                            + " dBm"); // Log in file
                                    ...
                                    if (agentsArray[ind_aux]
                                            .equals(vipAPAddr)) {
                                        System.out.print("[\033[48;5;94m"
                                                         + String.format(
                                                "%.2f", client_dBm[ind_aux])
                                                         + "\033[00m]"); // Orange
                                    } else {
                                    ...
                                    ps.println("\t\t[WorseAP] Rssi in agent "
                                               + agentsArray[ind_aux] + ": "
                                               + client_dBm[ind_aux]
                                               + " dBm"); // Log in file
                                }
                        ...
                        // End printf with colours

                        // this is used for all the modes except FF. If you also want a threshold in FF mode,
                        // substitute the next line with if(true){
                        // if(!SMARTAP_PARAMS.mode.equals("FF")){ // In BALANCER mode, it will assign STAs to
                        // APs always with higher RSSI than threshold, so there is not ping pong effect
                        if (true) { // In BALANCER mode, it will assign STAs to APs always with higher RSSI than
                            // threshold, so there is not ping pong effect
                            if (!agentsArray[client_index]
                                    .equals(agentAddr)) { // If the agent to which the STA is associated is not the one with
                                // the highest RSSI, change to the best RSSI
                                ...
                                if (currentRssi
                                    ...
                                    if ((handoffTime == null) || (
                                        ...
                                        handoffClientToAp(eth,
                                                          agentsArray[client_index]);
                                        handoffDate.put(eth,
                                                        System.currentTimeMillis()); // store the time for checking the hysteresis
                                        // next time
                                        System.out.println(
                                                " - Handoff >--->--->---> "
                                                + agentsArray[client_index]);
                                        ps.println(
                                                "\t\t[Action] Handoff to agent: "
                                                + agentsArray[client_index]); // Log in file

                                    } else {
                                        System.out.println(
                                                " - No Handoff: Hysteresis time not expired");
                                        ps.println(
                                                "\t\t[No Action] No Handoff: Hysteresis time not expired"); // Log in file
                                    }

                                } else {
                                    // The threshold is not reached
                                    switch (SMARTAP_PARAMS.mode) {
                                        case "RSSI":
                                            System.out.println(
                                                    " - No Handoff: Rssi Threshold not reached");
                                            ps.println(
                                                    "\t\t[No Action] No Handoff: Rssi Threshold not reached"); // Log in file

                                            break;
                                        case "DETECTOR":
                                            System.out.println(
                                                    " - Assigned by DETECTOR");
                                            ps.println(
                                                    "\t\t[No Action] No Handoff: Assigned by DETECTOR"); // Log in file
                                        ...
                                        case "FF":
                                            System.out.println(
                                                    " - Assigned by FF");
                                            ps.println(
                                                    "\t\t[No Action] No Handoff: Assigned by FF"); // Log in file
                                        ...
                                        default:
                                            ...
                                            ps.println(
                                                    "\t\t[No Action] No Handoff: Assigned by BALANCER"); // Log in file
                                ...
                                ps.println(
                                        "\t\t[No Action] There is no better Rssi heard"); // Log in file
                            }
                        }

                        // FF mode
                        if (SMARTAP_PARAMS.mode
                                .equals("FF")) { // Calculate FF data
                            System.out.println("\033[K\r[SmartAPSelection]");
                            ...
                            for (InetAddress agentAddrFF : agents) {
                                ...
                                if (oc.getLvap().getAgent().getIpAddress()
                                      .equals(agentAddrFF)) { // If the STA is associated to this agent, use the real
                                    // statistics
                                    // Reception statistics
                                    Map<MACAddress, Map<String, String>> vals_rx_FF = getRxStatsFromAgent(
                                            agentAddrFF);
                                    ...
                                    if (vals_entry_rx != null) {
                                        ...
                                        // vals_entry_rx.get("avg_rate") + " kbps");
                                        double clientRate = Double
                                                .parseDouble(vals_entry_rx
                                                                     .get("avg_rate"));
                                        // t and T
                                        double[] tTValues = getTransmissionTime(
                                                clientRate);
                                        double p = 0.98 * (tTValues[1]
                                                           / tTValues[0]);
                                        // System.out.println("\033[K\r[SmartAPSelection] Th_av["+ind_aux+"]: " +
                                        // String.format("%.2f",clientRate.doubleValue()*p));
                                        TH_av[ind_aux] = clientRate * p;
                                    } else {
                                        TH_av[ind_aux] = 0.0;
                                    }

                                } else { // if the STA is NOT associated to this agent, estimate the available
                                    // throughput
                                    ...
                                    HashSet<OdinClient> clients_FF = new HashSet<>(
                                            getClientsFromAgent(
                                                    agentAddrFF));
                                    double t2Value = calculateT2(
                                            clients_FF.size(), tTValues[0]);
                                    double p =
                                            0.98 * (tTValues[1] / t2Value);
                                    // System.out.println("\033[K\r[SmartAPSelection] Th_av["+ind_aux+"]: " +
                                    // String.format("%.2f",maxRate*p));
                        ...
                        System.out.println(
                                "\033[K\r[SmartAPSelection] No data received");
                    }
                }
                ...
                if (SMARTAP_PARAMS.mode
                        .equals("FF")) { // Show FF results and handoff if necessary
                    System.out.println(
                            "\033[K\r[SmartAPSelection] ====================");
                    System.out.println(showAPsLine
                                       + " - FF Throughput available [Mbps]\033[00m");
                    ...
                    for (OdinClient oc : clients) {
                        ...
                        if (clientAddr
                                .equals(nullAddr)) { // If client not assigned, next one
                            continue;
                        }
                        ...
                        Double maxFF = calculateFittingnessFactor(
                                SMARTAP_PARAMS.thReqSTA,
                                th_avFF[0]); // Start with first Th_av
                        Double currentTh_av = null;
                        System.out.print("\033[K\r[SmartAPSelection] ff["
                                         + ind_aux + "]=" + String
                                                 .format("%.3f", maxFF)
                                         + " ");

                        ArrayList<Integer> th_list = new ArrayList<>(); // To sort FF
                        th_list.add(0); // First FF

                        for (ind_aux = 1; ind_aux
                                          < th_avFF.length; ind_aux++) { // Get max position and calculate FF

                            Double currentFF = calculateFittingnessFactor(
                                    SMARTAP_PARAMS.thReqSTA,
                                    th_avFF[ind_aux]);
                            boolean added_FF = false;

                            for (int ind_list = 0;
                                 ind_list < th_list.size(); ind_list++) {
                            ...
                            System.out.print("ff[" + ind_aux + "]=" + String
                                    .format("%.3f", currentFF) + " ");
                        }
                        System.out.println();
                        System.out.print("\033[K\r[SmartAPSelection] ");
                        ...
                        for (ind_aux = 0;
                             ind_aux < th_avFF.length; ind_aux++) {
                            ...
                            if (agentsArray[ind_aux]
                                ...
                                if (currentTh_av != 0.0) {
                                    System.out.print("[\033[48;5;29;1m"
                                                     ...
                                                     + "\033[00m]"); // Dark Green
                                    ps.println(
                                            "\t\t[Associated] Throughput in agent "
                                            + agentsArray[ind_aux] + ": "
                                            + th_avFF[ind_aux]
                                            + " kbps"); // Log in file
                                } else {
                                    System.out.print("[\033[48;5;29;1m"
                                                     ...
                                                     + "\033[00m]"); // Dark Green
                                    ps.println(
                                            "\t\t[Associated] No packets received"); // Log in file
                                }
                                ...
                                if (ind_aux == client_index) { // Max
                                    ...
                                    System.out.print("[" + String
                                            .format("%.2f", th_avFF[ind_aux]
                                                            / 1000.0) + "]");
                                    ps.println(
                                            "\t\t[WorseAP] Throughput in agent "
                                            + agentsArray[ind_aux] + ": "
                                            + th_avFF[ind_aux]
                                            + " kbps"); // Log in file
                                }
                        ...
                        for (Integer integer : th_list) {
                            ...
                            if (!agentsArray[client_index]
                                ...
                                if (currentRssi
                                    ...
                                    if ((handoffTime == null) || (
                                        ...
                                        if (!(currentTh_av == 0.0)) {
                                            ...
                                            ps.println(
                                                    "\t\t[No Action] No packets received"); // Log in file
                                            break;
                                        }
                                    } else {
                                        System.out.println(
                                                "\033[0;1m - No Handoff: Hysteresis time not reached\033[00m");
                                        ps.println(
                                                "\t\t[No Action] No Handoff: Hysteresis time not reached"); // Log in file
                                        break;
                                    }
                                } else {
                                    // System.out.println(" - No Handoff: Rssi Threshold not reached");
                                    ps.println(
                                            "\t\t[No Action] No Handoff: Rssi Threshold not reached, try next FF"); // Log
                                    // in
                                    // file
                                }
                            } else {
                                System.out.println(); // Best AP already
                                ps.println(
                                        "\t\t[No Action] There is no better FF"); // Log in file
                                break;
                            }
                        }
                    }
                ...
                if ((SMARTAP_PARAMS.mode.equals("BALANCER"))
                    || (SMARTAP_PARAMS.mode.equals("JAIN-BALANCER"))) {

                    Map<MACAddress, InetAddress> assignedClients; // Array with the balancer decision MAC of the STA - IP of the AP
                    ...
                    if (SMARTAP_PARAMS.mode.equals("JAIN-BALANCER")) {
                        ...
                        System.out.println(showAPsLine
                                           + " - Jain's Fairness index Balancer\033[00m");
                        System.out.print("\033[K\r[SmartAPSelection] ");
                        assignedClients = jainsFairnessIndex(rssiData,
                                                             agentsArray,
                                                             clients,
                        ...
                        System.out.println(
                                showAPsLine + " - Balancer\033[00m");
                        System.out.print("\033[K\r[SmartAPSelection] ");
                        assignedClients = simpleBalancerAlgorithm(rssiData,
                                                                  agentsArray,
                                                                  clients,
                    ...
                    for (MACAddress eth : assignedClients.keySet()) {
                        ...
                        Long handoffTime = handoffDate.get(eth);
                        System.out.print("\033[K\r[SmartAPSelection] ");
                        OdinClient clientHandoff = getClientFromHwAddress(
                                eth);
                        ...
                        if ((handoffTime == null) || (
                            ...
                            InetAddress assignedAgent = assignedClients
                                    .get(eth);
                            ...
                            if (getClientFromHwAddress(eth) != null) {
                                ...
                                handoffDate.put(eth,
                                                System.currentTimeMillis());
                                System.out.print("\033[0;1mHandoff "
                                                 + clientHandoff
                                                         .getIpAddress()
                                                 + " >--->--->---> "
                                                 + assignedAgent
                                                 + "\033[00m");
                                ps.println("\t\t[Action] Handoff "
                                           + clientHandoff.getIpAddress()
                                           + " to agent: "
                                           + assignedAgent); // Log in file
                            }

                        } else {
                            System.out.print("No Handoff " + clientHandoff
                                    .getIpAddress()
                                             + ": Hysteresis time not expired");
                            ps.println(
                                    "\t\t[No Action] No Handoff: Hysteresis time not expired"); // Log in file
                        }
                        System.out.println();
                ...
                // DETECTOR mode
                if (SMARTAP_PARAMS.mode
                        .equals("DETECTOR")) { // If a flow is detected, the STA is moved to the VIP AP FIXME minimum rssi
                    System.out.println(
                            "\033[K\r[SmartAPSelection] ====================");
                    System.out.println(showAPsLine + " - DETECTOR - AP VIP: "
                                       + vipAPAddr + "\033[00m");
                    System.out.print("\033[K\r[SmartAPSelection] ");
                    printAgentsLoad(agentsArray,
                                    vipAPAddr); // Print load and VIP agent
                    System.out.println();
                    for (OdinClient oc : clients) { // All clients
                        MACAddress eth = oc.getMacAddress(); // client MAC
                        InetAddress clientAddr = oc
                                .getIpAddress(); // client IP

                        if (flowsReceived.containsKey(clientAddr)) {
                            DetectedFlow cntx = flowsReceived
                                    .get(clientAddr);
                        ...
                        System.out.print("\033[K\r\t[Flow] from agent: " + cntx.odinAgentAddr + " at " + cntx.timeStamp + "\n");*/
                            ...
                            if ((System.currentTimeMillis() - cntx.timeStamp)
                                > 30000) { // Clean flow after 30 sec
                                flowsReceived.remove(clientAddr);
                                System.out
                                        .print("\033[K\r\t[Flow] Clean flow from client "
                                               + clientAddr
                                               + " - Handoff\n");
                                handoffClientToAp(eth,
                                                  cntx.lastAgentAddr); // send the STA back to the AP where it was before the
                                // handoff to the VIP AP
                            } else {
                                InetAddress agentAddr = oc.getLvap()
                                                          .getAgent()
                                                          .getIpAddress();
                                if (!vipAPAddr.equals(agentAddr)) {
                                    ...
                                    if (client_dBm[vip_index]
                                        > SMARTAP_PARAMS.signal_threshold) {

                                        // move the STA to the VIP AP
                                        System.out
                                                .print("\033[K\r\t[Flow] Detected flow from client "
                                                       + clientAddr
                                                       + " - Handoff\n");
                    ...
                    // A new STA may have associated to the VIP AP. Remove it from there. This can be
                    // improved. FIXME
                    for (OdinClient oc : getClientsFromAgent(
                            vipAPAddr)) { // In case STA is associated before the app starts
                        MACAddress eth = oc.getMacAddress(); // client MAC
                        InetAddress clientAddr = oc
                                .getIpAddress(); // client IP
                ...
                ps.flush();
                System.out.println(
                        "\033[K\r[SmartAPSelection] Assignation done in: "
                        + (System.currentTimeMillis() - time) + " ms");
                System.out.println(
                        "\033[K\r[SmartAPSelection] ====================");
                ...
                Thread.sleep(
                        SMARTAP_PARAMS.pause); // If a pause or a period is needed
                System.out
                        .print("\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[K\r\n\033[0;0H"); // Clear lines above and return to console 0,0
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    private void softReset(Map<MACAddress, Double[]> rssiData,
                           Map<MACAddress, Long> handoffDate,
                           Map<MACAddress, Double[]> ffData) {
        System.out.println("\033[K\r[SmartAPSelection] Soft Resetting...");
        rssiData.clear();
        handoffDate.clear();
        ffData.clear();
    ...
    private Double getRssiFromRxStats(MACAddress clientMAC,
        ...
        for (String elem : arr.split("\n")) { // Split string in STAs
            ...
            if (clientMAC
                    .equals(eth)) { // If it belongs to the client, return rssi
    ...
    private double calculateT2(int numberOfStas, double tValue) {
        ...
        if (numberOfStas > 0) {
            ...
            double tCont =
                    (cwMin / 2) * slot * (1 + pcValue) / (2 * (numberOfStas
                                                               + 1));
    ...
    private double[] getTransmissionTime(
            double avgRate) { // Returns transmission times to calculate THavg

        double[] result;

        if (avgRate
            == 54000.0) { // If higher value not reached, we use the next lower values
    ...
    private double getOFDMRates(
            double SNR) { // Returns ODFM rates to calculate THavg

        double result = 0.0;

        if (SNR
            > 21.0) { // If higher value not reached, we use the next lower values
            result = 54000.0;
            return result;
        } else if (SNR >= 20.0) {
            result = 48000.0;
    ...
    private double calculateFittingnessFactor(double Rreq, double Rb) {
        ...
        if (Math.abs(Rb) > 1e-6) {
            double U =
                    Math.pow(Rb * shaping_mine / Rreq, shaping) / (1 + Math
                            .pow(Rb * shaping_mine / Rreq, shaping));
            double lambda = 1 - Math.pow(Math.E, -shaping_k / (
                    Math.pow(shaping - 1, 1 / shaping) + Math
                            .pow(shaping - 1, (1 - shaping) / shaping)));
            ff = (1 - Math.pow(Math.E,
                               -shaping_k * U / (Rb * shaping_mine / Rreq)))
                 / lambda;
        }
        return ff;
    }

    private Map<MACAddress, InetAddress> simpleBalancerAlgorithm(
            Map<MACAddress, Double[]> rssiData, InetAddress[] agentsArray,
            HashSet<OdinClient> clients,
            Double threshold) { // Print load in each AP and returns array of agents to assign
        ...
        for (InetAddress agentAddrBalancer : agentsArray) { // Create array with number of STAs for each AP, find the one with lower
            // number

            clients_Balancer = new HashSet<>(
            ...
            for (MACAddress eth : rssiData
                    .keySet()) { // If there is not a STA with enough RSSI, not handoff
                Double[] client_dBm = rssiData.get(eth);
        ...
        for (InetAddress ignored : agentsArray) { // Print APs and number of STAs associated at it
            ...
            if ((ind_aux == agent_index) && (numStasPerAgent[ind_aux]
                                             < meanStas) && (maxStas
                                                             > meanStas)) { // min<mean and max>mean try to handoff a STA
                System.out.print("[\033[48;5;88;1m  "
                                 + numStasPerAgent[ind_aux]
                                 + "   \033[00m]");
                orderHandoff = true;
            } else {
    ...
    private void handler(OdinEventFlowDetection oefd,
        ...
        DetectedFlow flowDetected = new DetectedFlow(IPSrcAddress,
                                                     IPDstAddress,
                                                     cntx.protocol,
                                                     cntx.SrcPort,
                                                     cntx.DstPort,
    ...
    public class DetectedFlow {
        ...
        public DetectedFlow(InetAddress IPSrcAddress,
                            InetAddress IPDstAddress, int protocol,
                            int SrcPort, int DstPort,
                            InetAddress odinAgentAddr,
                            InetAddress lastAgentAddr, long timeStamp) {
    ...
    private void printAgentsLoad(InetAddress[] agentsArray,
                                 InetAddress vipAgent) { // Print load in each AP
        ...
        for (InetAddress agentAddrAP : agentsArray) { // Print APs and number of STAs associated at it
            ...
            if (vipAgent.equals(agentAddrAP)) { // VIP agent
                if (numStasPerAgent[ind_aux] > 0) {
                    System.out.print("[\033[48;5;3;1m  "
                                     + numStasPerAgent[ind_aux]
                                     + "   \033[00m]");
                } else {
                    System.out.print("[\033[48;5;94m  "
    ...
    private Map<MACAddress, InetAddress> jainsFairnessIndex(
            Map<MACAddress, Double[]> rssiData, InetAddress[] agentsArray,
            HashSet<OdinClient> clients,
            Double threshold) { // Print load in each AP and returns array of agents to assign
        ...
        for (OdinClient oc : clients) { // For each STA
            ...
            if (client_dBm != null) {
                ...
                for (InetAddress agentAddrBalancer : agentsArray) { // For each AP
                    ...
                    if (clientAgent
                            .equals(agentAddrBalancer)) { // numSTAs not changed
                        ...
                        fairnessIndex[agentIndex.get(agentAddrBalancer)] =
                                num_clients / (num_agents * sum);
                    ...
                    if ((fairnessIndex[agentIndex.get(agentAddrBalancer)]
                         > maxFI) && (
                                client_dBm[agentIndex.get(agentAddrBalancer)]
                                > SMARTAP_PARAMS.signal_threshold)) {
                        maxFI = fairnessIndex[agentIndex
                                .get(agentAddrBalancer)];
                ...
                for (Double fair_index : fairnessIndex) {
                    ...
                    if (ind_aux == agent_index_assoc) { // Green
                        System.out.print("[\033[48;5;29;1m " + String
                                .format("%.2f", fair_index) + " \033[00m]");
                    } else if (ind_aux == index_maxFI) { // Red
                        System.out.print("[\033[48;5;88m " + String
                                .format("%.2f", fair_index) + " \033[00m]");
                    } else { // Other
                        System.out.print("[ " + String
                                .format("%.2f", fair_index) + " ]");
                    }
                    ind_aux++;
                }
                System.out.print(" - " + clientAddr);
                if ((client_dBm[index_maxFI]
                     > SMARTAP_PARAMS.signal_threshold) && (index_maxFI
                                                            != agent_index_assoc)) {
                    arrayHandoff.put(eth, agentsArray[index_maxFI]);
                    System.out
                            .println("\033[0;1m - Handoff ordered\033[00m");
