...
public class LearningSwitch
        implements IFloodlightModule, ILearningSwitchService,
        IOFMessageListener {
    ...
    protected void addToPortMap(IOFSwitch sw, long mac, short vlan,
        ...
        if (swMap == null) {
            ...
            swMap = Collections.synchronizedMap(
                    new LRULinkedHashMap<MacVlanPair, Short>(
                            MAX_MACS_PER_SWITCH));
    ...
    private void writeFlowMod(IOFSwitch sw, short command, int bufferId,
        ...
        flowMod.setOutPort((command == OFFlowMod.OFPFC_DELETE) ?
                           outPort :
                           OFPort.OFPP_NONE.getValue());
        flowMod.setFlags((command == OFFlowMod.OFPFC_DELETE) ?
                         0 :
    ...
    private Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi,
        ...
        if (outPort == null) {
            ...
            this.writeFlowMod(sw, OFFlowMod.OFPFC_ADD, pi.getBufferId(),
                              match, outPort);
    ...
    private Command processFlowRemovedMessage(IOFSwitch sw,
        ...
        if (flowRemovedMessage.getCookie()
            != LearningSwitch.LEARNING_SWITCH_COOKIE) {
            return Command.CONTINUE;
        }
        if (log.isTraceEnabled()) {
    ...
    public Command receive(IOFSwitch sw, OFMessage msg,
        ...
        switch (msg.getType()) {
            ...
            case FLOW_REMOVED:
                return this
                        .processFlowRemovedMessage(sw, (OFFlowRemoved) msg);
            case ERROR:
                log.info("received an error {} from switch {}",
                         (OFError) msg, sw);
                return Command.CONTINUE;
        }
