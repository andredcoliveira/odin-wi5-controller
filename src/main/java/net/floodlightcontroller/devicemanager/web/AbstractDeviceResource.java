...
public abstract class AbstractDeviceResource extends ServerResource {
    ...
    public Iterator<? extends IDevice> getDevices() {
        ...
        Iterator<Device> diter = (Iterator<Device>) deviceManager
                .queryDevices(macAddress, vlan, ipv4Address, switchDPID,
                              switchPort);

        final String macStartsWith = form
                .getFirstValue("mac__startswith", true);
        final String vlanStartsWith = form
                .getFirstValue("vlan__startswith", true);
        final String ipv4StartsWith = form
                .getFirstValue("ipv4__startswith", true);
        final String dpidStartsWith = form
                .getFirstValue("dpid__startswith", true);
        final String portStartsWith = form
                .getFirstValue("port__startswith", true);

        return new FilterIterator<Device>(diter) {
            @Override protected boolean matches(Device value) {
                if (macStartsWith != null) {
                    if (!value.getMACAddressString()
                              .startsWith(macStartsWith))
                        return false;
                }
                ...
                if (ipv4StartsWith != null) {
                    ...
                    for (Integer v : value.getIPv4Addresses()) {
                        ...
                        if (v != null && str.startsWith(ipv4StartsWith)) {
                            match = true;
                            break;
                        }
                    }
                ...
                if (dpidStartsWith != null) {
                    ...
                    for (SwitchPort v : value.getAttachmentPoints(true)) {
                        String str = HexString
                                .toHexString(v.getSwitchDPID(), 8);
                        if (v != null && str.startsWith(dpidStartsWith)) {
                            match = true;
                            break;
                        }
                    }
                ...
                if (portStartsWith != null) {
                    ...
                    for (SwitchPort v : value.getAttachmentPoints(true)) {
                        ...
                        if (v != null && str.startsWith(portStartsWith)) {
                            match = true;
                            break;
                        }
                    }
