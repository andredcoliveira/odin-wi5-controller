...
public class Forwarding extends ForwardingBase implements IFloodlightModule {
        ...
        if (dstDevice != null) {
            ...
            for (SwitchPort dstDap : dstDevice.getAttachmentPoints()) {
                ...
                if ((dstIsland != null) && dstIsland.equals(srcIsland)) {
                    on_same_island = true;
                    if ((sw.getId() == dstSwDpid) && (pi.getInPort()
                                                      == dstDap.getPort())) {
                        on_same_if = true;
                    }
                    break;
                ...
                if (srcVsDest == 0) {
                    if (!srcDap.equals(dstDap) && (srcCluster != null) && (
                        ...
                        if (route != null) {
                            ...
                            long cookie = AppCookie
                                    .makeCookie(FORWARDING_APP_ID, 0);
    ...
    protected void doFlood(IOFSwitch sw, OFPacketIn pi,
                           FloodlightContext cntx) {
        if (topology.isIncomingBroadcastAllowed(sw.getId(), pi.getInPort())
            == false) {
            if (log.isTraceEnabled()) {
