...
public class SwitchClustersResource extends ServerResource {
    @Get("json") public Map<String, List<String>> retrieve() {
        ...
        for (Entry<Long, IOFSwitch> entry : floodlightProvider.getSwitches()
                                                              .entrySet()) {
            Long clusterDpid = topology
                    .getL2DomainId(entry.getValue().getId());
            List<String> switchesInCluster = switchClusterMap
                    .get(HexString.toHexString(clusterDpid));
