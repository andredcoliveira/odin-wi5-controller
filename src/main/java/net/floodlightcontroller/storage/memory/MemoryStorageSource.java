...
public class MemoryStorageSource extends NoSqlStorageSource {
    ...
    synchronized private MemoryTable getTable(String tableName,
                                              boolean create) {
    ...
    @Override
    protected Collection<Map<String, Object>> getAllRows(String tableName,
    ...
    @Override
    protected Map<String, Object> getRow(String tableName,
                                         String[] columnNameList,
                                         Object rowKey) {
        MemoryTable table = getTable(tableName, false);
    ...
    @Override
    protected List<Map<String, Object>> executeEqualityQuery(
        ...
        synchronized (table) {
            ...
            for (Map<String, Object> row : allRows) {
                ...
                if (value != null) {
                    ...
                    if ((compareResult > 0) || (startInclusive && (
                            compareResult >= 0))) {
    ...
    @Override
    protected void insertRows(String tableName,
    ...
    @Override
    protected void updateRows(String tableName, Set<Object> rowKeys,
        ...
        synchronized (table) {
            for (Object rowKey : rowKeys) {
                ...
                for (Map.Entry<String, Object> entry : updateRowList
                        .entrySet()) {
    ...
    protected void updateRowsImpl(String tableName,
        ...
        synchronized (table) {
            for (Map<String, Object> updateRow : updateRowList) {
                ...
                if (row == null)
                    row = table.newRow(rowKey);
                for (Map.Entry<String, Object> entry : updateRow
                        .entrySet()) {
                    row.put(entry.getKey(), entry.getValue());
                }
            }
