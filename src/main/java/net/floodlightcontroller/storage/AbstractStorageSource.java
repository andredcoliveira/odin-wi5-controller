...
public abstract class AbstractStorageSource
    ...
    protected static Logger logger = LoggerFactory
            .getLogger(AbstractStorageSource.class);
    ...
    protected static ExecutorService defaultExecutorService = Executors
    ...
    private Map<String, Set<IStorageSourceListener>> listeners = new ConcurrentHashMap<String, Set<IStorageSourceListener>>();
    ...
    abstract class StorageCallable<V> implements Callable<V> {
        public V call() {
            try {
                ...
                logger.error("Failure in asynchronous call to executeQuery",
                             e);
    ...
    abstract class StorageRunnable implements Runnable {
        public void run() {
            try {
                ...
                logger.error("Failure in asynchronous call to updateRows",
                             e);
                if (exceptionHandler != null)
                    exceptionHandler.handleException(e);
                throw e;
    ...
    @Override
    public abstract void setTablePrimaryKeyName(String tableName,
    ...
    @Override public Set<String> getAllTableNames() {
    ...
    protected void updateCounters(String baseName, String tableName) {
        if (counterStore != null) {
            ...
            if (tableName != null) {
                ...
                counterName =
                        baseName + CounterStore.TitleDelimitor + tableName;
            } else {
            ...
            if (counter == null) {
                counter = counterStore
                        .createCounter(counterName, CounterType.LONG);
            }
            counter.increment();
        }
    }
    ...
    @Override
    public abstract IQuery createQuery(String tableName,
                                       String[] columnNames,
                                       IPredicate predicate,
                                       RowOrdering ordering);

    @Override public IResultSet executeQuery(IQuery query) {
    ...
    public IResultSet executeQuery(String tableName, String[] columnNames,
                                   IPredicate predicate,
                                   RowOrdering ordering) {
        IQuery query = createQuery(tableName, columnNames, predicate,
                                   ordering);
    ...
    public Object[] executeQuery(String tableName, String[] columnNames,
        ...
        IResultSet resultSet = executeQuery(tableName, columnNames,
                                            predicate, ordering);
    ...
    public Future<IResultSet> executeQueryAsync(final IQuery query) {
        Future<IResultSet> future = executorService
                .submit(new StorageCallable<IResultSet>() {
    ...
    public Future<Object[]> executeQueryAsync(final String tableName,
                                              ...
                                              final IPredicate predicate,
                                              final RowOrdering ordering,
                                              final IRowMapper rowMapper) {
        Future<Object[]> future = executorService
                .submit(new StorageCallable<Object[]>() {
                    public Object[] doStorageOperation() {
                        return executeQuery(tableName, columnNames,
                                            predicate, ordering, rowMapper);
                    }
                });
        return future;
    }
    ...
    public Future<?> insertRowAsync(final String tableName,
        ...
        Future<?> future = executorService.submit(new StorageRunnable() {
    ...
    @Override
    public Future<?> deleteRowAsync(final String tableName,
                                    final Object rowKey) {
        Future<?> future = executorService.submit(new StorageRunnable() {
            public void doStorageOperation() {
    ...
    @Override
    public Future<?> deleteRowsAsync(final String tableName,
                                     final Set<Object> rowKeys) {
        Future<?> future = executorService.submit(new StorageRunnable() {
            public void doStorageOperation() {
    ...
    @Override
    public Future<?> deleteMatchingRowsAsync(final String tableName,
                                             final IPredicate predicate) {
        Future<?> future = executorService.submit(new StorageRunnable() {
            public void doStorageOperation() {
    ...
    @Override
    public Future<?> getRowAsync(final String tableName,
                                 final Object rowKey) {
        Future<?> future = executorService.submit(new StorageRunnable() {
            public void doStorageOperation() {
    ...
    @Override public Future<?> saveAsync(final IResultSet resultSet) {
        Future<?> future = executorService.submit(new StorageRunnable() {
            public void doStorageOperation() {
                resultSet.save();
            }
    ...
    protected abstract void insertRowImpl(String tableName,
    ...
    protected abstract void updateRowsImpl(String tableName,
                                           List<Map<String, Object>> rows);
    ...
    protected abstract void updateMatchingRowsImpl(String tableName,
                                                   IPredicate predicate,
                                                   Map<String, Object> values);
    ...
    protected abstract void updateRowImpl(String tableName,
                                          Map<String, Object> values);

    @Override public void deleteRow(String tableName, Object rowKey) {
    ...
    @Override public void deleteRows(String tableName, Set<Object> rowKeys) {
        updateCounters(STORAGE_DELETE_COUNTER_NAME, tableName);
        deleteRowsImpl(tableName, rowKeys);
    }

    protected abstract void deleteRowsImpl(String tableName,
                                           Set<Object> rowKeys);
    ...
    @Override public IResultSet getRow(String tableName, Object rowKey) {
        updateCounters(STORAGE_QUERY_COUNTER_NAME, tableName);
        return getRowImpl(tableName, rowKey);
    }

    protected abstract IResultSet getRowImpl(String tableName,
                                             Object rowKey);

    @Override
    public synchronized void addListener(String tableName,
                                         IStorageSourceListener listener) {
        Set<IStorageSourceListener> tableListeners = listeners
                .get(tableName);
    ...
    @Override
    public synchronized void removeListener(String tableName,
                                            IStorageSourceListener listener) {
        Set<IStorageSourceListener> tableListeners = listeners
                .get(tableName);
        if (tableListeners != null) {
    ...
    public void notifyListeners(
            List<StorageSourceNotification> notifications) {
