...
public class CounterStore
     ...
     */
    protected Map<String, CounterEntry> nameToCEIndex = new ConcurrentHashMap<String, CounterEntry>();
     ...
     */
    protected static Map<NetworkLayer, Map<String, List<String>>> layeredCategories = new ConcurrentHashMap<NetworkLayer, Map<String, List<String>>>();
    ...
    public void updatePacketInCounters(IOFSwitch sw, OFMessage m,
        ...
        if (TypeAliases.l3TypeAliasMap != null && TypeAliases.l3TypeAliasMap
                .containsKey(etherType)) {
        ...
        String controllerCounterName = CounterStore
                .createCounterName(CONTROLLER_NAME, -1, packetName);

        String controllerL3CategoryCounterName = CounterStore
                .createCounterName(CONTROLLER_NAME, -1, packetName,
                                   etherType, NetworkLayer.L3);
        ...
        String controllerL2CategoryCounterName = CounterStore
                .createCounterName(CONTROLLER_NAME, -1, packetName, l2Type,
                                   NetworkLayer.L2);
        String switchL2CategoryCounterName = CounterStore
                .createCounterName(switchIdHex, -1, packetName, l2Type,
                                   NetworkLayer.L2);
        String portL2CategoryCounterName = CounterStore
                .createCounterName(switchIdHex, packet.getInPort(),
                                   packetName, l2Type, NetworkLayer.L2);

        // Construct both port and switch L3 counter for the packet_in
        String portCounterName = CounterStore
                .createCounterName(switchIdHex, packet.getInPort(),
                                   packetName);
        String switchCounterName = CounterStore
                .createCounterName(switchIdHex, -1, packetName);

        String portL3CategoryCounterName = CounterStore
                .createCounterName(switchIdHex, packet.getInPort(),
                                   packetName, etherType, NetworkLayer.L3);
        String switchL3CategoryCounterName = CounterStore
                .createCounterName(switchIdHex, -1, packetName, etherType,
                                   NetworkLayer.L3);
        ...
        try {
            ...
            // L3 counters
            ICounter controllerL3Counter = getCounter(
                    controllerL3CategoryCounterName);
            if (controllerL3Counter == null) {
                controllerL3Counter = createCounter(
            ...
            ICounter switchL3Counter = getCounter(
                    switchL3CategoryCounterName);
            ...
            if (etherType.compareTo(CounterStore.L3ET_IPV4) == 0) {
                ...
                String controllerL4CategoryCounterName = CounterStore
                        .createCounterName(CONTROLLER_NAME, -1, packetName,
                                           l4Type, NetworkLayer.L4);
                String portL4CategoryCounterName = CounterStore
                        .createCounterName(switchIdHex, packet.getInPort(),
                                           packetName, l4Type,
                                           NetworkLayer.L4);
                String switchL4CategoryCounterName = CounterStore
                        .createCounterName(switchIdHex, -1, packetName,
                                           l4Type, NetworkLayer.L4);
                ICounter controllerL4Counter = getCounter(
                        controllerL4CategoryCounterName);
                if (controllerL4Counter == null) {
                    controllerL4Counter = createCounter(
                            controllerL4CategoryCounterName,
                            CounterType.LONG);
                }
                controllerL4Counter.increment();
                ICounter portL4Counter = getCounter(
                        portL4CategoryCounterName);
                if (portL4Counter == null) {
                    portL4Counter = createCounter(portL4CategoryCounterName,
                                                  CounterType.LONG);
                }
                portL4Counter.increment();
                ICounter switchL4Counter = getCounter(
                        switchL4CategoryCounterName);
                if (switchL4Counter == null) {
                    switchL4Counter = createCounter(
                            switchL4CategoryCounterName, CounterType.LONG);
                }
                switchL4Counter.increment();
            }
        } catch (IllegalArgumentException e) {
            log.error("Invalid Counter, " + portCounterName + " or "
                      + switchCounterName);
        }
    }
    ...
    public void updatePktOutFMCounterStore(IOFSwitch sw, OFMessage ofMsg) {
        ...
        String controllerFMCounterName = CounterStore
                .createCounterName(CONTROLLER_NAME, -1, packetName);
        try {
            ICounter counter = getCounter(controllerFMCounterName);
            if (counter == null) {
                counter = createCounter(controllerFMCounterName,
        ...
        String switchFMCounterName = CounterStore
                .createCounterName(sw.getStringId(), -1, packetName);
        try {
            ICounter counter = getCounter(switchFMCounterName);
            if (counter == null) {
                counter = createCounter(switchFMCounterName,
                                        CounterValue.CounterType.LONG);
            }
            counter.increment();
        } catch (IllegalArgumentException e) {
     ...
     */
    public static String createCounterName(String switchID, int portID,
                                           String counterName) {
        if (portID < 0) {
            return switchID + TitleDelimitor + counterName;
        } else {
            return switchID + TitleDelimitor + portID + TitleDelimitor
                   + counterName;
        }
    }
    ...
    public static String createCounterName(String switchID, int portID,
        ...
        if (portID < 0) {
            ...
            fullCounterName =
                    groupCounterName + TitleDelimitor + subCategory;
        } else {
            groupCounterName =
                    switchID + TitleDelimitor + portID + TitleDelimitor
                    + counterName;
            fullCounterName =
                    groupCounterName + TitleDelimitor + subCategory;
        }
    ...
    public List<String> getAllCategories(String counterName,
                                         NetworkLayer layer) {
        if (layeredCategories.containsKey(layer)) {
            Map<String, List<String>> counterToCategories = layeredCategories
                    .get(layer);
    ...
    public ICounter createCounter(String key,
        ...
        if (!nameToCEIndex.containsKey(key)) {
            ...
            throw new IllegalArgumentException(
                    "Title for counters must be unique, and there is already a counter with title "
                    + key);
        }
        ...
        Executors.newScheduledThreadPool(1)
                 .scheduleAtFixedRate(new Runnable() {
                     public void run() {
                         ...
                         randomCounter.increment(new Date(),
                                                 (long) (Math.random()
                                                         * 100)); //TODO - pull this in to random timing
                     }
    ...
    @Override public Map<String, ICounter> getAll() {
        ...
        for (Map.Entry<String, CounterEntry> counterEntry : this.nameToCEIndex
                .entrySet()) {
