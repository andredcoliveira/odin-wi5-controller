...
public class ConcurrentCounter implements ICounter {
    ...
    static {
        liveCounters = Collections.newSetFromMap(
                new ConcurrentHashMap<ConcurrentCounter, Boolean>()); //nifty way to get concurrent hash set
    ...
    protected void init(Date startDate) {
        ...
        for (DateSpan ds : DateSpan.values()) {
            CountBuffer cb = new CountBuffer(startDate, ds,
                                             MAX_HISTORY.get(ds));
            counts.put(ds, cb);
        }
    }
    ...
    @Override public void increment(Date d, long delta) {
        this.unprocessedCountBuffer.add(new CountAtom(d, delta));
    }

    @Override public void setCounter(Date d, CounterValue value) {
        // To be done later
    }
     ...
     * Reset the value.
     */
    @Override public void reset(Date startDate) {
        init(startDate);
    }
    ...
    public synchronized void flush() {
        for (CountAtom c = this.unprocessedCountBuffer.poll();
             c != null; c = this.unprocessedCountBuffer.poll()) {
            for (DateSpan ds : DateSpan.values()) {
    ...
    @Override public CounterValue getCounterValue() {
        // To be done later
    ...
    @Override public Date getCounterDate() {
        // To be done later
        //CountSeries cs = counts.get(DateSpan.REALTIME).snapshot();
        //return cs.getSeries()[0];
        return new Date();
