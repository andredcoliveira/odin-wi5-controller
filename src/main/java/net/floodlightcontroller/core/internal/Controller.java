...
public class Controller
        implements IFloodlightProviderService, IStorageSourceListener {
    ...
    public void setStorageSourceService(
     ...
     */
    protected ChannelUpstreamHandler getChannelHandler(
    ...
    protected class OFChannelHandler
        ...
        public void exceptionCaught(ChannelHandlerContext ctx,
            ...
            if (e.getCause() instanceof ReadTimeoutException) {
                ...
                log.error("Disconnecting switch {} due to IO Error: {}", sw,
        ...
        void processSwitchDescReply() {
            try {
                ...
                Future<List<OFStatistics>> desc_future = (Future<List<OFStatistics>>) sw
                        .
                                getAttribute(
                                        IOFSwitch.SWITCH_DESCRIPTION_FUTURE);
                List<OFStatistics> values = desc_future
                ...
                if (values != null) {
                    ...
                    try {
                        ...
                        resultSet = storageSource
                                .getRow(SWITCH_CONFIG_TABLE_NAME, swid);
                        for (Iterator<IResultSet> it = resultSet
                                .iterator(); it.hasNext(); ) {
        ...
        protected boolean handleVendorMessage(OFVendor vendorMessage) {
            ...
            switch (vendor) {
                case OFNiciraVendorData.NX_VENDOR_ID:
                    ...
                    break;
                default:
                    log.warn("Unhandled VENDOR message; vendor id = {}",
                             vendor);
                    break;
            }
        ...
        protected void processOFMessage(OFMessage m)
            ...
            switch (m.getType()) {
                ...
                case GET_CONFIG_REPLY:
                    if (!state.hsState
                            .equals(HandshakeState.FEATURES_REPLY)) {
                ...
                case ERROR:
                    ...
                    if (sw.checkFirstPendingRoleRequestXid(error.getXid())) {
                        ...
                        if (isBadVendorError) {
                            ...
                            state.firstRoleReplyReceived = true;
                            sw.deliverRoleRequestNotSupported(
                                    error.getXid());
                            synchronized (roleChanger) {
                                if (sw.role == null
    ...
    protected void handlePortStatusMessage(IOFSwitch sw, OFPortStatus m,
        ...
        } else if (m.getReason() == (byte) OFPortReason.OFPPR_ADD
                .ordinal()) {
            sw.setPort(port);
            if (updateStorage)
                updatePortInfo(sw, port);
            log.debug("Port #{} added for {}", portNumber, sw);
        } else if (m.getReason() == (byte) OFPortReason.OFPPR_DELETE
                .ordinal()) {
    ...
    protected static final ThreadLocal<Stack<FloodlightContext>> flcontext_cache = new ThreadLocal<Stack<FloodlightContext>>() {
        @Override protected Stack<FloodlightContext> initialValue() {
        ...
        switch (et) {
            case OFPET_HELLO_FAILED:
                ...
                log.error("Error {} {} from {}",
                          new Object[] { et, hfc, sw });
                break;
            case OFPET_BAD_REQUEST:
                OFBadRequestCode brc = OFBadRequestCode.values()[0xffff
                                                                 & error.getErrorCode()];
                log.error("Error {} {} from {}",
                          new Object[] { et, brc, sw });
                break;
            case OFPET_BAD_ACTION:
                OFBadActionCode bac = OFBadActionCode.values()[0xffff & error
                        .getErrorCode()];
                log.error("Error {} {} from {}",
                          new Object[] { et, bac, sw });
                break;
            case OFPET_FLOW_MOD_FAILED:
                OFFlowModFailedCode fmfc = OFFlowModFailedCode.values()[
                        0xffff & error.getErrorCode()];
                log.error("Error {} {} from {}",
                          new Object[] { et, fmfc, sw });
                break;
            case OFPET_PORT_MOD_FAILED:
                OFPortModFailedCode pmfc = OFPortModFailedCode.values()[
                        0xffff & error.getErrorCode()];
                log.error("Error {} {} from {}",
                          new Object[] { et, pmfc, sw });
                break;
            case OFPET_QUEUE_OP_FAILED:
                OFQueueOpFailedCode qofc = OFQueueOpFailedCode.values()[
                        0xffff & error.getErrorCode()];
                log.error("Error {} {} from {}",
                          new Object[] { et, qofc, sw });
                break;
            default:
                break;
        }
    ...
    protected void addSwitch(IOFSwitch sw) {
        ...
        OFSwitchImpl oldSw = (OFSwitchImpl) this.activeSwitches
                .put(sw.getId(), sw);
        if (sw == oldSw) {
    ...
    protected void removeSwitch(IOFSwitch sw) {
        ...
        if (!this.activeSwitches.remove(sw.getId(), sw) || !sw
                .isConnected()) {
    ...
    @Override public Map<Long, IOFSwitch> getSwitches() {
        return Collections.unmodifiableMap(this.activeSwitches);
    }

    @Override public void addOFSwitchListener(IOFSwitchListener listener) {
    ...
    @Override public Map<OFType, List<IOFMessageListener>> getListeners() {
    ...
    public boolean injectOfMessage(IOFSwitch sw, OFMessage msg,
        ...
        if (sw == null) {
            log.info("Failed to inject OFMessage {} onto a null switch",
                     msg);
            return false;
        }
    ...
    @Override public boolean injectOfMessage(IOFSwitch sw, OFMessage msg) {
    ...
    @Override public BasicFactory getOFMessageFactory() {
        return factory;
    }
    ...
    protected void updateAllInactiveSwitchInfo() {
        ...
        try {
            OperatorPredicate op = new OperatorPredicate(
                    SWITCH_CONTROLLER_ID, OperatorPredicate.Operator.EQ,
                    controllerId);
            switchResultSet = storageSource
                    .executeQuery(SWITCH_TABLE_NAME, switchColumns, op,
                                  null);
            while (switchResultSet.next()) {
                ...
                try {
                    ...
                    op = new OperatorPredicate(PORT_SWITCH,
                                               ...
                                               datapathId);
                    portResultSet = storageSource
                            .executeQuery(PORT_TABLE_NAME, portColumns, op,
                                          null);
    ...
    protected void updatePortInfo(IOFSwitch sw, OFPhysicalPort port) {
        ...
        String hardwareAddressString = HexString
                .toHexString(hardwareAddress);
    ...
    protected void readSwitchPortStateFromStorage(OFSwitchImpl sw) {
        OperatorPredicate op = new OperatorPredicate(PORT_SWITCH,
                                                     OperatorPredicate.Operator.EQ,
                                                     sw.getStringId());
        IResultSet portResultSet = storageSource
                .executeQuery(PORT_TABLE_NAME, null, op, null);
        ...
        while (portResultSet.next()) {
            try {
                ...
                String portMac = portResultSet
                        .getString(PORT_HARDWARE_ADDRESS);
    ...
    @Override public void addHAListener(IHAListener listener) {
        this.haListeners.add(listener);
    }

    @Override public void removeHAListener(IHAListener listener) {
    ...
    protected void handleControllerNodeIPChanges() {
        ...
        synchronized (controllerNodeIPsCache) {
            ...
            IResultSet res = storageSource
                    .executeQuery(CONTROLLER_INTERFACE_TABLE_NAME, colNames,
                                  null, null);
            while (res.next()) {
                if (res.getString(CONTROLLER_INTERFACE_TYPE)
                       .equals("Ethernet")
    ...
    @Override public Map<String, String> getControllerNodeIPs() {
    ...
    @Override public long getSystemStartTime() {
