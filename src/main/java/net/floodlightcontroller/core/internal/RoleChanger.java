...
public class RoleChanger {
    ...
    protected static class RoleChangeTask implements Delayed {
        ...
        public RoleChangeTask(Collection<OFSwitchImpl> switches, Role role,
                              long deadline) {
        ...
        @Override public long getDelay(TimeUnit tu) {
    ...
    protected class RoleRequestWorker extends Thread {
        @Override public void run() {
            ...
            try {
                ...
                log.error(
                        "RoleRequestWorker task had an uncaught exception. "
    ...
    public synchronized void submitRequest(Collection<OFSwitchImpl> switches,
                                           Role role) {
        ...
        ArrayList<OFSwitchImpl> switches_copy = new ArrayList<OFSwitchImpl>(
                switches);
        RoleChangeTask req = new RoleChangeTask(switches_copy, role,
                                                deadline);
    ...
    protected void sendRoleRequest(Collection<OFSwitchImpl> switches,
        ...
        while (iter.hasNext()) {
            ...
            try {
                Boolean supportsNxRole = (Boolean) sw
                ...
                if ((supportsNxRole == null) || supportsNxRole) {
                    ...
                    if (role == Role.SLAVE) {
                        log.debug(
                                "Disconnecting switch {} that doesn't support "
    ...
    protected void verifyRoleReplyReceived(Collection<OFSwitchImpl> switches,
        ...
        for (OFSwitchImpl sw : switches) {
            if (sw.checkFirstPendingRoleRequestCookie(cookie)) {
                sw.getChannel().close();
                log.warn(
                        "Timeout while waiting for role reply from switch {}."
                        + " Disconnecting", sw);
            }
        }
    }
