...
public class OFMessageFilterManager
    ...
    protected static Logger log = LoggerFactory
            .getLogger(OFMessageFilterManager.class);
    ...
    protected String addFilter(ConcurrentHashMap<String, String> f,
                               long delta) {
        ...
        for (i = 0; i < MAX_FILTERS; ++i) {
            ...
            if (!filterMap.containsKey(s))
                break;  // implies you can use this key for session id.
        }
        ...
        filterMap.put(s, f);
        if (filterTimeoutMap.containsKey(s))
            filterTimeoutMap.remove(s);
        filterTimeoutMap.put(s, delta);

        if (filterMap.size()
            == 1) { // set the timer as there will be no existing timers.
    ...
    public String setupFilter(String sid,
        ...
        if (sid == null) {
            ...
            log.debug("Adding new filter: {} for {} seconds", f,
                      deltaInSecond);
    ...
    protected String refreshFilter(String s, int delta) {
        ...
        if (t != null) {
            ...
            if (t > MAX_FILTER_TIME)
                t = MAX_FILTER_TIME;
            filterTimeoutMap.put(s, t);
            return SUCCESS;
        } else
            return FILTER_SESSION_ID_NOT_FOUND;
    }
    ...
    protected String deleteFilter(String sessionId) {
        ...
        if (filterMap.containsKey(sessionId)) {
            ...
            log.debug("Deleted Filter {}.  # of filters remaining: {}",
                      sessionId, filterMap.size());
            return SUCCESS;
        } else
            return FILTER_SESSION_ID_NOT_FOUND;
    }

    public HashSet<String> getMatchedFilters(OFMessage m,
        ...
        while (filterIt.hasNext()) {   // for every filter
            ...
            while (fieldIt.hasNext()) {
                ...
                if (filterFieldType.equals("mac")) {
                    ...
                    String srcMac = HexString
                            .toHexString(eth.getSourceMACAddress());
                    String dstMac = HexString
                            .toHexString(eth.getDestinationMACAddress());
                    log.debug("srcMac: {}, dstMac: {}", srcMac, dstMac);

                    if (filterFieldValue.equals(srcMac) || filterFieldValue
                            .equals(dstMac)) {
                        filterMatch = true;
                    } else {
                        filterMatch = false;
    ...
    public boolean connectToPSServer() {
        ...
        while (numRetries++ < MaxRetry) {
            try {
                transport = new TFramedTransport(
                        new TSocket("localhost", serverPort));
                transport.open();
                ...
                log.debug(
                        "Have a connection to packetstreamer server localhost:{}",
                        serverPort);
                break;
            } catch (TException x) {
                try {
        ...
        if (numRetries > MaxRetry) {
            log.error(
                    "Failed to establish connection with the packetstreamer server.");
            return false;
        }
        return true;
    }
    ...
    public void disconnectFromPSServer() {
        if (transport != null && transport.isOpen()) {
            log.debug(
                    "Close the connection to packetstreamer server localhost:{}",
                    serverPort);
            transport.close();
        }
    }
    ...
    public Command receive(IOFSwitch sw, OFMessage msg,
        ...
        if (log.isDebugEnabled()) {
            log.debug("Received packet {} from switch {}", msg,
                      sw.getStringId());
        }
    ...
    protected void sendPacket(HashSet<String> matchedFilters, IOFSwitch sw,
                              ...
                              boolean sync) throws TException {
        ...
        packet.setMessageType(
                OFMessageType.findByValue((msg.getType().ordinal())));

        switch (msg.getType()) {
            case PACKET_IN:
                OFPacketIn pktIn = (OFPacketIn) msg;
                packet.setSwPortTuple(
                        new SwitchPortTuple(sw.getId(), pktIn.getInPort()));
            ...
            case PACKET_OUT:
                ...
                packet.setSwPortTuple(
                        new SwitchPortTuple(sw.getId(), pktOut.getInPort()));
            ...
            default:
                packet.setSwPortTuple(
                        new SwitchPortTuple(sw.getId(), (short) 0));
    ...
    @Override public void startUp(FloodlightModuleContext context) {
        ...
        serverPort = Integer.parseInt(System.getProperty(
                "net.floodlightcontroller.packetstreamer.port", "9090"));
