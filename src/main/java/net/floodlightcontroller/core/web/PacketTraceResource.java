...
public class PacketTraceResource extends ServerResource {
    protected static Logger log = LoggerFactory
            .getLogger(PacketTraceResource.class);
    ...
    public static class FilterParameters {
        ...
        public String toString() {
            return "SessionID: " + sessionId + "\tmac" + mac + "\tperiod"
                   + period + "\tdirection" + direction + "\toutput"
                   + output;
        }
    }
    ...
    @Post("json") public PacketTraceOutput packettrace(FilterParameters fp) {

        ConcurrentHashMap<String, String> filter = new ConcurrentHashMap<String, String>();
        String sid = null;
        PacketTraceOutput output = new PacketTraceOutput();
        OFMessageFilterManager manager = (OFMessageFilterManager) getContext()
                .getAttributes().
        ...
        if (filter.isEmpty()) {
            ...
            log.debug("Call setupFilter: sid:{} filter:{}, period:{}",
                      new Object[] { fp.getSessionId(), filter,
                                     fp.getPeriod() });
            sid = manager
                    .setupFilter(fp.getSessionId(), filter, fp.getPeriod());
