...
public class SwitchCounterCategoriesResource extends CounterResourceBase {
    @Get("json") public Map<String, Object> retrieve() {
        ...
        String counterName = (String) getRequestAttributes()
                .get("counterName");
        String layer = (String) getRequestAttributes().get("layer");

        Long[] switchDpids;
        if (switchID.equalsIgnoreCase("all")) {
            switchDpids = floodlightProvider.getSwitches().keySet()
                                            .toArray(new Long[0]);
            for (Long dpid : switchDpids) {
                switchID = HexString.toHexString(dpid);

                getOneSwitchCounterCategoriesJson(model, switchID,
                                                  counterName, layer);
            }
        } else {
            getOneSwitchCounterCategoriesJson(model, switchID, counterName,
                                              layer);
        }
    ...
    protected void getOneSwitchCounterCategoriesJson(
            Map<String, Object> model, String switchID, String counterName,
            String layer) {
        ...
        try {
            ...
            fullCounterName = switchID + ICounterStoreService.TitleDelimitor
                              + counterName;
        ...
        List<String> categories = this.counterStore
                .getAllCategories(fullCounterName, nl);
        if (categories != null) {
