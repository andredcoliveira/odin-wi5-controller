...
public abstract class ForwardingBase
    ...
    protected static Logger log = LoggerFactory
            .getLogger(ForwardingBase.class);
    ...
    public TimedCache<Long> broadcastCache = new TimedCache<Long>(100, 5
    ...
    public Comparator<SwitchPort> clusterIdComparator = new Comparator<SwitchPort>() {
        @Override public int compare(SwitchPort d1, SwitchPort d2) {
     ...
     */
    public abstract Command processPacketInMessage(IOFSwitch sw,
                                                   OFPacketIn pi,
    ...
    public boolean pushRoute(Route route, OFMatch match,
                             Integer wildcard_hints, int bufferId,
                             OFPacketIn pi, long pinSwitch, long cookie,
                             FloodlightContext cntx,
        ...
        fm.setIdleTimeout((short) 5).setBufferId(OFPacketOut.BUFFER_ID_NONE)
          .setCookie(cookie).setCommand(flowModCommand).setMatch(match)
          .setActions(actions).setLengthU(
                OFFlowMod.MINIMUM_LENGTH + OFActionOutput.MINIMUM_LENGTH);
        ...
        for (int indx = switchPortList.size() - 1; indx > 0; indx -= 2) {
            ...
            if (1 == indx) {
                ...
                if ((reqeustFlowRemovedNotifn) && (match.getDataLayerType()
                                                   != Ethernet.TYPE_ARP)) {
    ...
    public void PacketOutMultiPort(OFPacketIn pi, IOFSwitch sw, short inPort,
        ...
        while (j.hasNext()) {
            actions.add(
                    new OFActionOutput(j.next().shortValue(), (short) 0));
        }
        ...
        po.setActions(actions);
        po.setActionsLength(
                (short) (OFActionOutput.MINIMUM_LENGTH * outPorts.size()));
    ...
    protected boolean isInBroadcastCache(IOFSwitch sw, OFPacketIn pi,
        ...
        Long broadcastHash;
        broadcastHash =
                topology.getL2DomainId(sw.getId()) * prime + eth.hashCode();
    ...
    protected boolean isInSwitchBroadcastCache(IOFSwitch sw, OFPacketIn pi,
        ...
        return sw.updateBroadcastCache(new Long(eth.hashCode()),
                                       pi.getInPort());
    ...
    public static boolean blockHost(
        ...
        match.setDataLayerSource(Ethernet.toByteArray(host_mac))
             .setInputPort((short) inputPort).setWildcards(
                OFMatch.OFPFW_ALL & ~OFMatch.OFPFW_DL_SRC
                & ~OFMatch.OFPFW_IN_PORT);
        fm.setCookie(cookie).setHardTimeout((short) hardTimeout)
          .setIdleTimeout((short) 5).setBufferId(OFPacketOut.BUFFER_ID_NONE)
          .setMatch(match).setActions(actions).setLengthU(
                OFFlowMod.MINIMUM_LENGTH); // +OFActionOutput.MINIMUM_LENGTH);
    ...
    @Override public void deviceAdded(IDevice device) {
        // NOOP
    }

    @Override public void deviceRemoved(IDevice device) {
        // NOOP
    }
    ...
    @Override public void deviceMoved(IDevice device) {
        ...
        match.setDataLayerDestination(
                Ethernet.toByteArray(device.getMACAddress()));
