...
public class VirtualNetworkFilter
        implements IFloodlightModule, IVirtualNetworkService,
        IOFMessageListener, IDeviceListener {
    protected static Logger log = LoggerFactory
            .getLogger(VirtualNetworkFilter.class);
    ...
    @Override public void deleteNetwork(String guid) {
        String name = null;
        if (nameToGuid.isEmpty()) {
            log.warn(
    ...
    @Override public void addHost(MACAddress mac, String guid, String port) {
        if (guid != null) {
            if (log.isDebugEnabled()) {
            ...
            log.warn(
                    "Could not add MAC {} to network ID {} on port {}, the network does not exist",
                    new Object[] { mac, guid, port });
        }
    }

    @Override public void deleteHost(MACAddress mac, String port) {
        if (log.isDebugEnabled()) {
        ...
        if (port != null) {
            ...
            if (!portToMac.isEmpty()) {
                for (Entry<String, MACAddress> entry : portToMac
                        .entrySet()) {
                    if (entry.getValue().equals(mac)) {
                        portToMac.remove(entry.getKey());
     ...
     */
    protected Command processPacketIn(IOFSwitch sw, OFPacketIn msg,
                                      FloodlightContext cntx) {
        ...
        if (eth.isBroadcast() || eth.isMulticast() || isDefaultGateway(eth)
            || isDhcpPacket(eth)) {
            ...
            ret = Command.STOP;
        } else if (oneSameNetwork(eth.getSourceMAC(),
        ...
        if (log.isTraceEnabled())
            log.trace("Results for flow between {} and {} is {}",
                      new Object[] { eth.getSourceMAC(),
                                     eth.getDestinationMAC(), ret });
        /*
    ...
    protected void doDropFlow(IOFSwitch sw, OFPacketIn pi,
        ...
        if (sw == null) {
            log.warn(
                    "Switch is null, not installing drop flowmod for PacketIn {}",
                    pi);
            return;
        }
        ...
        fm.setCookie(cookie).setIdleTimeout(FLOW_MOD_DEFAULT_IDLE_TIMEOUT)
          .setHardTimeout((short) 0).setBufferId(OFPacketOut.BUFFER_ID_NONE)
