...
public class StaticFlowEntryPusher
        ...
        IStaticFlowEntryPusherService, IStorageSourceListener,
    ...
    public static String ColumnNames[] = { COLUMN_NAME, COLUMN_SWITCH,
                                           ...
                                           COLUMN_IDLE_TIMEOUT,
                                           COLUMN_HARD_TIMEOUT,
                                           COLUMN_PRIORITY, COLUMN_COOKIE,
                                           COLUMN_WILDCARD, COLUMN_IN_PORT,
                                           COLUMN_DL_SRC, COLUMN_DL_DST,
                                           COLUMN_DL_VLAN,
                                           COLUMN_DL_VLAN_PCP,
                                           COLUMN_DL_TYPE, COLUMN_NW_TOS,
                                           COLUMN_NW_PROTO, COLUMN_NW_SRC,
    ...
    protected void sendEntriesToSwitch(IOFSwitch sw) {
        ...
        if ((entriesFromStorage != null) && (entriesFromStorage
                .containsKey(dpid))) {
            ...
            for (String entryName : sortedList) {
                ...
                if (flowMod != null) {
                    if (log.isDebugEnabled()) {
                        log.debug("Pushing static entry {} for {}", dpid,
                                  entryName);
                    }
    ...
    private Map<String, Map<String, OFFlowMod>> readEntriesFromStorage() {
        ...
        try {
            ...
            IResultSet resultSet = storageSource
    ...
    void parseRow(Map<String, Object> row,
        ...
        if (!row.containsKey(COLUMN_SWITCH) || !row
                .containsKey(COLUMN_NAME)) {
            log.error(
        ...
        try {
            ...
            for (String key : row.keySet()) {
                ...
                if (key.equals(COLUMN_HARD_TIMEOUT) || key
                        .equals(COLUMN_IDLE_TIMEOUT) || key
                            .equals(COLUMN_WILDCARD))
                    continue;
                if (key.equals(COLUMN_ACTIVE)) {
                    if (!Boolean.valueOf((String) row.get(COLUMN_ACTIVE))) {
                        ...
                        entries.get(switchName).put(entryName,
                                                    null);  // mark this an inactive
                        return;
                    }
                } else if (key.equals(COLUMN_ACTIONS)) {
                    StaticFlowEntries.parseActionString(flowMod, (String) row
                            .get(COLUMN_ACTIONS), log);
                } else if (key.equals(COLUMN_COOKIE)) {
                    flowMod.setCookie(StaticFlowEntries
                    ...
                    flowMod.setPriority(U16.t(Integer.valueOf(
                            (String) row.get(COLUMN_PRIORITY))));
            ...
            if (entryName != null && switchName != null)
                log.error(
                        "skipping entry {} on switch {} with bad data : " + e
                                .getMessage(), entryName, switchName);
            else
    ...
    @Override public void rowsModified(String tableName,
                                       Set<Object> rowKeys) {
        ...
        for (String dpid : entriesToAdd.keySet()) {
            if (!entriesFromStorage.containsKey(dpid))
                entriesFromStorage
                        .put(dpid, new HashMap<String, OFFlowMod>());
            ...
            for (String entry : entriesToAdd.get(dpid).keySet()) {
                ...
                OFFlowMod oldFlowMod = entriesFromStorage.get(dpid)
                                                         .get(entry);
    ...
    public void rowsDeleted(String tableName, Set<Object> rowKeys) {
        ...
        for (Object obj : rowKeys) {
            if (!(obj instanceof String)) {
                log.error("tried to delete non-string key {}; ignoring",
                          obj);
                continue;
            }
    ...
    private boolean deleteStaticFlowEntry(String entryName) {
        ...
        if (entriesFromStorage.containsKey(dpid) && entriesFromStorage
                .get(dpid).containsKey(entryName)) {
     ...
     */
    private void writeOFMessagesToSwitch(long dpid,
        ...
        if (ofswitch != null) {  // is the switch connected
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Sending {} new entries to {}",
                              messages.size(), dpid);
                }
                ofswitch.write(messages, null);
                ofswitch.flush();
            } catch (IOException e) {
                log.error("writed to write to switch {} but got {}", dpid,
                          e.getMessage());
            }
        }
    }
    ...
    public Command receive(IOFSwitch sw, OFMessage msg,
        ...
        if (AppCookie.extractApp(cookie) == STATIC_FLOW_APP_ID) {
            if (flowRemoved.getReason()
                ...
                log.error(
                        "PANIC -- got a FlowRemove message for a infinite timeout flow: {} from switch {}",
                        msg, sw);
    ...
    @Override public void addFlow(String name, OFFlowMod fm, String swDpid) {
        Map<String, Object> fmMap = StaticFlowEntries
                .flowModToStorageEntry(fm, swDpid, name);
        entry2dpid.put(name, swDpid);
        Map<String, OFFlowMod> switchEntries = entriesFromStorage
                .get(swDpid);
    ...
    @Override public void deleteFlow(String name) {
        storageSource.deleteRowAsync(TABLE_NAME, name);
        // TODO - What if there is a delay in storage?
    }

    @Override public void deleteAllFlows() {
        for (String entry : entry2dpid.keySet()) {
            deleteFlow(entry);
        }
    }

    @Override public void deleteFlowsForSwitch(long dpid) {
    ...
    @Override public Map<String, Map<String, OFFlowMod>> getFlows() {
        return entriesFromStorage;
    }

    @Override public Map<String, OFFlowMod> getFlows(String dpid) {
