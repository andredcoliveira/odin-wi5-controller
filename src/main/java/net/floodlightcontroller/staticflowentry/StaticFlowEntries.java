...
public class StaticFlowEntries {
    protected static Logger log = LoggerFactory
            .getLogger(StaticFlowEntries.class);
    ...
    private static byte[] zeroMac = new byte[] { 0x0, 0x0, 0x0, 0x0, 0x0,
                                                 0x0 };
    ...
    public static long computeEntryCookie(OFFlowMod fm, int userCookie,
        ...
        return AppCookie.makeCookie(StaticFlowEntryPusher.STATIC_FLOW_APP_ID,
                                    flowHash);
    }
     ...
     */
    public static String getEntryNameFromJson(String fmJson)
            throws IOException {
     ...
     */
    public static Map<String, Object> flowModToStorageEntry(OFFlowMod fm,
                                                            String sw,
                                                            String name) {
        ...
        entry.put(StaticFlowEntryPusher.COLUMN_ACTIVE,
                  Boolean.toString(true));
        entry.put(StaticFlowEntryPusher.COLUMN_PRIORITY,
                  Short.toString(fm.getPriority()));
        entry.put(StaticFlowEntryPusher.COLUMN_WILDCARD,
                  Integer.toString(match.getWildcards()));

        if ((fm.getActions() != null) && (fm.getActions().size() > 0))
            entry.put(StaticFlowEntryPusher.COLUMN_ACTIONS, StaticFlowEntries
                    .flowModActionsToString(fm.getActions()));

        if (match.getInputPort() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_IN_PORT,
                      Short.toString(match.getInputPort()));

        if (!Arrays.equals(match.getDataLayerSource(), zeroMac))
            entry.put(StaticFlowEntryPusher.COLUMN_DL_SRC,
                      HexString.toHexString(match.getDataLayerSource()));

        if (!Arrays.equals(match.getDataLayerDestination(), zeroMac))
            entry.put(StaticFlowEntryPusher.COLUMN_DL_DST, HexString
                    .toHexString(match.getDataLayerDestination()));

        if (match.getDataLayerVirtualLan() != -1)
            entry.put(StaticFlowEntryPusher.COLUMN_DL_VLAN,
                      Short.toString(match.getDataLayerVirtualLan()));

        if (match.getDataLayerVirtualLanPriorityCodePoint() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_DL_VLAN_PCP,
                      Short.toString(
                              match.getDataLayerVirtualLanPriorityCodePoint()));

        if (match.getDataLayerType() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_DL_TYPE,
                      Short.toString(match.getDataLayerType()));

        if (match.getNetworkTypeOfService() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_NW_TOS,
                      Short.toString(match.getNetworkTypeOfService()));

        if (match.getNetworkProtocol() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_NW_PROTO,
                      Short.toString(match.getNetworkProtocol()));

        if (match.getNetworkSource() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_NW_SRC,
                      IPv4.fromIPv4Address(match.getNetworkSource()));

        if (match.getNetworkDestination() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_NW_DST,
                      IPv4.fromIPv4Address(match.getNetworkDestination()));

        if (match.getTransportSource() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_TP_SRC,
                      Short.toString(match.getTransportSource()));

        if (match.getTransportDestination() != 0)
            entry.put(StaticFlowEntryPusher.COLUMN_TP_DST,
                      Short.toString(match.getTransportDestination()));
    ...
    private static String flowModActionsToString(List<OFAction> fmActions) {
        ...
        for (OFAction a : fmActions) {
            ...
            switch (a.getType()) {
                case OUTPUT:
                    sb.append("output=" + Short
                            .toString(((OFActionOutput) a).getPort()));
                    break;
                case OPAQUE_ENQUEUE:
                    int queue = ((OFActionEnqueue) a).getQueueId();
                    short port = ((OFActionEnqueue) a).getPort();
                    sb.append("enqueue=" + Short.toString(port) + ":0x"
                              + String.format("%02x", queue));
                    break;
                case STRIP_VLAN:
                    sb.append("strip-vlan");
                    break;
                case SET_VLAN_ID:
                    sb.append("set-vlan-id=" + Short.toString(
                            ((OFActionVirtualLanIdentifier) a)
                                    .getVirtualLanIdentifier()));
                    break;
                case SET_VLAN_PCP:
                    sb.append("set-vlan-priority=" + Byte.toString(
                            ((OFActionVirtualLanPriorityCodePoint) a)
                                    .getVirtualLanPriorityCodePoint()));
                    break;
                case SET_DL_SRC:
                    sb.append("set-src-mac=" + HexString.toHexString(
                            ((OFActionDataLayerSource) a)
                                    .getDataLayerAddress()));
                    break;
                case SET_DL_DST:
                    sb.append("set-dst-mac=" + HexString.toHexString(
                            ((OFActionDataLayerDestination) a)
                                    .getDataLayerAddress()));
                    break;
                case SET_NW_TOS:
                    sb.append("set-tos-bits=" + Byte.toString(
                            ((OFActionNetworkTypeOfService) a)
                                    .getNetworkTypeOfService()));
                    break;
                case SET_NW_SRC:
                    sb.append("set-src-ip=" + IPv4.fromIPv4Address(
                            ((OFActionNetworkLayerSource) a)
                                    .getNetworkAddress()));
                    break;
                case SET_NW_DST:
                    sb.append("set-dst-ip=" + IPv4.fromIPv4Address(
                            ((OFActionNetworkLayerDestination) a)
                                    .getNetworkAddress()));
                    break;
                case SET_TP_SRC:
                    sb.append("set-src-port=" + Short.toString(
                            ((OFActionTransportLayerSource) a)
                                    .getTransportPort()));
                    break;
                case SET_TP_DST:
                    sb.append("set-dst-port=" + Short.toString(
                            ((OFActionTransportLayerDestination) a)
                                    .getTransportPort()));
                    break;
                default:
     ...
     * @throws IOException If there was an error parsing the JSON
     */
    public static Map<String, Object> jsonToStorageEntry(String fmJson)
            throws IOException {
        Map<String, Object> entry = new HashMap<String, Object>();
        MappingJsonFactory f = new MappingJsonFactory();
        JsonParser jp;
        ...
        if (actionstr != null) {
            ...
            for (String subaction : actionstr.split(",")) {
                ...
                if (action.equals("output")) {
                    subaction_struct = StaticFlowEntries
                            .decode_output(subaction, log);
    ...
    private static SubActionStruct decode_output(String subaction,
        ...
        n = Pattern.compile(
                "output=(?:((?:0x)?\\d+)|(all)|(controller)|(local)|(ingress-port)|(normal)|(flood))")
                   .matcher(subaction);
        if (n.matches()) {
            ...
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug("  Invalid port in: '{}' (error ignored)",
                              subaction);
                    return null;
                }
    ...
    private static SubActionStruct decode_enqueue(String subaction,
        ...
        n = Pattern.compile("enqueue=(?:((?:0x)?\\d+)\\:((?:0x)?\\d+))")
                   .matcher(subaction);
        if (n.matches()) {
            short portnum = 0;
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug("  Invalid port-num in: '{}' (error ignored)",
                              subaction);
                    return null;
                }
            }
            ...
            if (n.group(2) != null) {
                try {
                    ...
                    log.debug("  Invalid queue-id in: '{}' (error ignored)",
                              subaction);
                    return null;
                }
            }
    ...
    private static SubActionStruct decode_set_vlan_id(String subaction,
                                                      Logger log) {
        SubActionStruct sa = null;
        Matcher n = Pattern.compile("set-vlan-id=((?:0x)?\\d+)")
                           .matcher(subaction);
        ...
        if (n.matches()) {
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug("  Invalid VLAN in: {} (error ignored)",
                              subaction);
                    return null;
                }
            }
    ...
    private static SubActionStruct decode_set_vlan_priority(String subaction,
                                                            Logger log) {
        SubActionStruct sa = null;
        Matcher n = Pattern.compile("set-vlan-priority=((?:0x)?\\d+)")
                           .matcher(subaction);
        ...
        if (n.matches()) {
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug(
                            "  Invalid VLAN priority in: {} (error ignored)",
                            subaction);
                    return null;
                }
            }
    ...
    private static SubActionStruct decode_set_src_mac(String subaction,
        ...
        Matcher n = Pattern.compile(
                "set-src-mac=(?:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+))")
                           .matcher(subaction);
    ...
    private static SubActionStruct decode_set_dst_mac(String subaction,
        ...
        SubActionStruct sa = null;
        Matcher n = Pattern.compile(
                "set-dst-mac=(?:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+)\\:(\\p{XDigit}+))")
                           .matcher(subaction);
    ...
    private static SubActionStruct decode_set_tos_bits(String subaction,
                                                       Logger log) {
        SubActionStruct sa = null;
        Matcher n = Pattern.compile("set-tos-bits=((?:0x)?\\d+)")
                           .matcher(subaction);
        ...
        if (n.matches()) {
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug("  Invalid dst-port in: {} (error ignored)",
                              subaction);
                    return null;
                }
            }
    ...
    private static SubActionStruct decode_set_src_port(String subaction,
                                                       Logger log) {
        SubActionStruct sa = null;
        Matcher n = Pattern.compile("set-src-port=((?:0x)?\\d+)")
                           .matcher(subaction);
        ...
        if (n.matches()) {
            if (n.group(1) != null) {
                try {
                    ...
                    log.debug("  Invalid src-port in: {} (error ignored)",
                              subaction);
                    return null;
                }
            }
    ...
    private static SubActionStruct decode_set_dst_port(String subaction,
                                                       Logger log) {
        SubActionStruct sa = null;
        Matcher n = Pattern.compile("set-dst-port=((?:0x)?\\d+)")
                           .matcher(subaction);
        ...
        for (int i = 0; i < 6; i++) {
            if (n.group(i + 1) != null) {
                try {
                    ...
                    log.debug("  Invalid src-mac in: '{}' (error ignored)",
                              subaction);
                    return null;
                }
            } else {
                log.debug("  Invalid src-mac in: '{}' (null, error ignored)",
                          subaction);
                return null;
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            if (n.group(i + 1) != null) {
                try {
                    ...
                    log.debug("  Invalid src-ip in: '{}' (error ignored)",
                              subaction);
                    return 0;
                }
            } else {
                log.debug("  Invalid src-ip in: '{}' (null, error ignored)",
                          subaction);
                return 0;
            }
        }
